...
object Camera {
    ...
    fun setBooleanFeature(
        supportAllMenu: Boolean = false,
        disableTemperatureCheck: Boolean = false,
        supportFrameWatermark: Boolean = false,
        supportBodyBeauty: Boolean = false,
    ) {
        if (param.packageName != Package.CAMERA) {
            return
        }
        if (!supportAllMenu && !disableTemperatureCheck && !supportFrameWatermark && !supportBodyBeauty) {
            return
        }
        afterAttach {
            ...
            if (hookConfig != null) {
                setBooleanFeature(
                    hookConfig,
                    supportAllMenu,
                    disableTemperatureCheck,
                    supportFrameWatermark,
                    supportBodyBeauty
                )
            }
        }
    ...
    private fun Context.setBooleanFeature(
        ...
        val interceptor = XposedInterface.Hooker { chain ->
            ...
            try {
                ...
                val enableEnums = mutableListOf<BooleanFeatureEnum>()
                if (supportAllMenu) {
                    enableEnums.addAll(
                        listOf(
                ...
                if (supportFrameWatermark) {
                    enableEnums.add(BooleanFeatureEnum.SUPPORT_FRAME_WATERMARK)
                    enableEnums.add(BooleanFeatureEnum.SUPPORT_WATERMARK_FONT_SAMSUNG_SHARP_SANS)
                }
                if (supportBodyBeauty) {
                    enableEnums.add(BooleanFeatureEnum.SUPPORT_BODY_BEAUTY)
                }
                for (myEnum in enableEnums) {
                    val enum = try {
