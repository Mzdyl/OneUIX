...
object Gallery {
    ...
    context(xposedModule: XposedModule, param: XposedModuleInterface.PackageReadyParam)
    fun supportGoogleSync() {
        if (param.packageName != Package.GALLERY) return
        val classLoader = param.classLoader

        try {
            val featureClass = classLoader.loadClass("com.samsung.android.gallery.support.utils.Features")
            val supportGCloud = featureClass.reflect["SUPPORT_GCLOUD"]
            if (supportGCloud != null) {
                xposedModule.hook(
                    supportGCloud.javaClass.getDeclaredMethod("getEnabling")
                ).intercept { true }
            }
        } catch (t: Throwable) {
            xlog(t)
        ...
        try {
            val settingPreferenceClass = classLoader.loadClass(
                "com.samsung.android.gallery.module.settings.SettingPreference"
            )
            val googleCloudSync = settingPreferenceClass.reflect["GoogleCloudSync"]?.javaClass
            if (googleCloudSync != null) {
                xposedModule.hook(
                    googleCloudSync.getDeclaredMethod("support", Context::class.java)
                ).intercept { true }
            }
        } catch (t: Throwable) {
            xlog(t)
        ...
        try {
            val settingCloudClass = classLoader.loadClass(
                "com.samsung.android.gallery.settings.ui.SettingCloud"
            )
            settingCloudClass.declaredConstructors.forEach { ctor ->
                xposedModule.hook(ctor).intercept { chain ->
                    val result = chain.proceed()
                    runCatching { chain.thisObject.reflect["SUPPORT_CLOUD"] = true }
                    result
                }
            }
        } catch (t: Throwable) {
        ...
        try {
            val cloudStateCompatClass = classLoader.loadClass(
                "com.samsung.android.gallery.module.cloud.CloudStateCompat"
            )
            xposedModule.hook(
                cloudStateCompatClass.getDeclaredMethod("isGCloudUISupported")
            ).intercept { true }
        } catch (_: Throwable) {}

        try {
            val cloudCompatClass = classLoader.loadClass(
                "com.samsung.android.gallery.module.cloud.CloudCompat"
            )
            xposedModule.hook(
                cloudCompatClass.getDeclaredMethod("isGCloudUISupported")
            ).intercept { true }
        } catch (_: Throwable) {}

        try {
            val accountStatusClass = classLoader.loadClass(
                "com.samsung.android.gallery.module.cloud.CloudCompat\$AccountStatus"
            )
            xposedModule.hook(
                accountStatusClass.getDeclaredMethod("isSupportedCountry", Context::class.java)
            ).intercept { true }
        } catch (_: Throwable) {}
    }

    context(xposedModule: XposedModule, param: XposedModuleInterface.PackageReadyParam)
    fun applyFeatureOverrides(other: Preference.Other) {
        if (param.packageName != Package.GALLERY) return
        val classLoader = param.classLoader
        ...
        if (other.supportGalleryCloudSyncToolbar) {
            try {
                val featureClass = classLoader.loadClass("com.samsung.android.gallery.support.utils.Features")
                val syncToolbar = featureClass.reflect["SUPPORT_CLOUD_SYNC_MENU_ON_TOOL_BAR"]
                if (syncToolbar != null) {
                    xposedModule.hook(
                        syncToolbar.javaClass.getDeclaredMethod("getEnabling")
                    ).intercept { true }
                }
            } catch (t: Throwable) {
                xlog(t)
        ...
        if (other.enableGalleryPoi) {
            try {
                val featureClass = classLoader.loadClass("com.samsung.android.gallery.support.utils.Features")
                val supportPoi = featureClass.reflect["SUPPORT_POI"]
                if (supportPoi != null) {
                    xposedModule.hook(
                        supportPoi.javaClass.getDeclaredMethod("getEnabling")
                    ).intercept { true }
                }
            } catch (t: Throwable) {
                xlog(t)
        ...
        if (pocNeedsHook) {
            try {
                val pocFeaturesClass = classLoader.loadClass(
                    "com.samsung.android.gallery.support.utils.PocFeatures"
                )
                xposedModule.hook(
                    pocFeaturesClass.getDeclaredMethod("isEnabled")
                ).intercept { chain ->
                    val name = (chain.thisObject as? Enum<*>)?.name
                        ?: runCatching { chain.thisObject.reflect.call("name") as? String }.getOrNull()
                    when (name) {
                        "GalleryLabsDev" -> if (other.unlockGalleryDeveloperLabs) true else chain.proceed()
                        "UndoDelete" -> if (other.enableGalleryUndoDelete) true else chain.proceed()
                        "DualPhotoPreview", "CompareImages" -> if (other.enableGalleryDualPhotoPreview) true else chain.proceed()
                        "DoubleTapSeek" -> if (other.enableGalleryQuickSeek) true else chain.proceed()
                        "ThumbnailPreviewHdr" -> if (other.enableGalleryHdrThumbnail) true else chain.proceed()
                        "PhotoStripHighQualityPreview" -> if (other.enableGalleryPhotoStripHighQuality) true else chain.proceed()
                        "SetAudioUnMuteAlways" -> if (other.enableGalleryUnmuteAlways) true else chain.proceed()
                        "FullAddressDisplay" -> if (other.enableGalleryFullAddress) true else chain.proceed()
                        else -> chain.proceed()
                    }
                }
            } catch (t: Throwable) {
