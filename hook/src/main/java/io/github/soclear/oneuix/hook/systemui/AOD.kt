...
object AOD {
    ...
    fun aodLockSupportLunar() {
        ...
        try {
            ...
            val boolHooker = XposedInterface.Hooker { chain ->
                if (chain.args[0] == "CscFeature_Calendar_EnableLunar") {
                    true
                } else {
                    chain.proceed()
                }
            ...
            listOf(
                semCscFeatureClass.getDeclaredMethod("getBoolean", String::class.java),
                semCscFeatureClass.getDeclaredMethod("getBoolean", String::class.java, Boolean::class.javaPrimitiveType)
            ).forEach { method ->
                xposedModule.hook(method).intercept(boolHooker)
            }
        } catch (t: Throwable) {
            xlog(t)
        }
